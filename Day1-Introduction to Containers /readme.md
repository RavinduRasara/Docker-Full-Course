...
A **container** is a lightweight, standalone package that includes an application and everything it needs to run—like code, runtime, libraries, and dependencies.  
It ensures the application runs the same way everywhere, whether on a developer’s laptop, a server, or the cloud.

### 1.2 Containerization  
**Containerization** is the process of packaging an application along with its dependencies into a container. This allows applications to run consistently across different environments.
...
### 1.4 Evolution of Deployment Technology  
The evolution of deployment technology can be summarized as:

Servers → Virtual Machines → Containers

- **Virtual Machines** are an advancement over **Physical Servers**.  
- **Containers** are an advancement over **Virtual Machines**.


### 1.4.1 Physical Servers (1980s–2000s)

Physical servers were actual hardware machines where one server ran a single application.

Physical Server (Hardware) > OS (Linux/Windows) > Application 1 > Its Dependencies
...
Virtual Machines are software-based simulations of physical computers running on top of a physical server.
...
### 1.2 Difference Between Docker and Virtualization
...
![Dockerarchitecture](images/Dockerarchitecture2.png)
